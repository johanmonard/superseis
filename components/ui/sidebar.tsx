...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("app-scrollbar flex-1 overflow-auto p-[var(--space-2)]", className)}
    {...props}
  />
));
...
export const SidebarItem = React.forwardRef<HTMLButtonElement, SidebarItemProps>(
  ({ className, active = false, collapsed = false, type = "button", ...props }, ref) => (
    <button
      ...
      className={cn(
        "inline-flex h-[var(--control-height-md)] w-full items-center rounded-[var(--radius-sm)] border px-[var(--space-4)] text-sm font-medium transition-[background-color,border-color,color,box-shadow] duration-150 ease-out",
